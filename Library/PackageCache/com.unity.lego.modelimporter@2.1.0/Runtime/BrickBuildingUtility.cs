...
namespace LEGOModelImporter
{
    public class BrickBuildingUtility
    {
        ...
        private const float maxRayDistance = 250.0f;
        ...
        /// <summary>
        /// Align a vector to LU_5 in local space of a building plane
        /// </summary>
        ...
        public static bool GetGridAlignedPosition(Ray ray, Plane worldPlane, PhysicsScene physicsScene, out RaycastHit collidingHit)
        {
            var hits = physicsScene.Raycast(ray.origin, ray.direction, raycastBuffer, maxRayDistance, ~LayerMask.GetMask(Connection.connectivityFeatureLayerName), QueryTriggerInteraction.Ignore);
            if(hits > 0)
            {
        ...
        /// <returns></returns>
        public static bool IsCollidingAtTransformation(Brick brick, Vector3 position, Quaternion rotation, HashSet<Brick> ignoreList = null)
        {
            ...
            {
                var isColliding = Part.IsColliding(part, colliderBuffer, ignoreList);
                if (isColliding)
                {
        ...
        public static Bounds ComputeBounds(HashSet<Brick> bricks, Matrix4x4 transformation)
        {
            if(bricks.Count == 0)
            {
                return new Bounds();
        ...
        /// <returns></returns>
        public static Bounds ComputeBounds(HashSet<Brick> bricks)
        {
        ...
        Vector3 pickupOffset, Ray ray, Plane fallbackWorldPlane, out Vector3 offset, out Vector3 alignedOffset, out Quaternion rotation, out RaycastHit hit)
        {
            ...
            if(GetGridAlignedPosition(ray, fallbackWorldPlane, sourceBrick.gameObject.scene.GetPhysicsScene(), out hit))
            {                
                ...
                var brickRight = sourceBrick.transform.right;
                var brickUp = sourceBrick.transform.up;
                var brickForward = sourceBrick.transform.forward;
                var brickLeft = -brickRight;
                var brickDown = -brickUp;
                var brickBack = -brickForward;

                var rightAngle = Vector3.Angle(brickRight, normal);
                var upAngle = Vector3.Angle(brickUp, normal);
                var forwardAngle = Vector3.Angle(brickForward, normal);
                var leftAngle = Vector3.Angle(brickLeft, normal);
                var downAngle = Vector3.Angle(brickDown, normal);
                var backAngle = Vector3.Angle(brickBack, normal);

                var axisAngles = new (Vector3, float)[6]{(brickRight, rightAngle), (brickUp, upAngle), (brickForward, forwardAngle), 
                (brickLeft, leftAngle), (brickDown, downAngle), (brickBack, backAngle)};
                var chosenIndex = -1;
                ...
                Quaternion rot = Quaternion.FromToRotation(axisAngles[chosenIndex].Item1, normal);                
                var cachedSourceRot = sourceBrick.transform.rotation;
                sourceBrick.transform.rotation = rot * sourceBrick.transform.rotation;

                var axesToAlign = new List<Vector3>();
                if(chosenIndex == 0) // Right was aligned
                {
                    axesToAlign.Add(sourceBrick.transform.up);
                    axesToAlign.Add(sourceBrick.transform.forward);
                }
                else if(chosenIndex == 1) // Up was aligned
                {
                    axesToAlign.Add(sourceBrick.transform.right);
                    axesToAlign.Add(sourceBrick.transform.forward);
                }
                else if(chosenIndex == 2) // Forward was aligned
                {
                    axesToAlign.Add(sourceBrick.transform.up);
                    axesToAlign.Add(sourceBrick.transform.right);
                }
                else if(chosenIndex == 3) // Left was aligned
                {
                    axesToAlign.Add(-sourceBrick.transform.up);
                    axesToAlign.Add(-sourceBrick.transform.forward);
                }
                else if(chosenIndex == 4) // Down was aligned
                {
                    axesToAlign.Add(-sourceBrick.transform.right);
                    axesToAlign.Add(-sourceBrick.transform.forward);
                }
                else if(chosenIndex == 5) // Back was aligned
                {
                    axesToAlign.Add(-sourceBrick.transform.up);
                    axesToAlign.Add(-sourceBrick.transform.right);
                }
                ...
                if(hit.collider != null)
                {
                    ...
                    rot = MathUtils.AlignRotation(axesToAlign.ToArray(), m) * rot;
                ...
                {
                    rot = MathUtils.AlignRotation(axesToAlign.ToArray(), Matrix4x4.identity) * rot;
            ...
            else
            {
                ...
                var pointOnRay = ray.GetPoint(maxRayDistance);
        ...
        private static List<Brick> CastBrick(HashSet<Brick> castBricks, Ray ray, Brick[] bricksToCheck)
        {
            ...
            var distanceToBrick = Vector3.Distance(ray.origin, totalBounds.center); // Use this to get the radius of the cone
            ...
            MathUtils.Cone cone = new MathUtils.Cone(ray.origin, ray.direction, angle);
        ...
        private static bool Colliding(HashSet<Brick> bricks, Brick ignore)
        {
            foreach (var selected in bricks)
            {
                ...
                {
                    if (Part.IsColliding(selectedPart, colliderBuffer, bricks))
                    {
                        return true;
                    }
                }
