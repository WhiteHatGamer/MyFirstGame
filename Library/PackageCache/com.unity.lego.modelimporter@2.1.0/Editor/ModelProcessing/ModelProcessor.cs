...
namespace LEGOModelImporter
{
    ...
    public static class ModelProcessor
    {
        ...
        {
            bool collapseMesh = (group.type != GroupType.Dynamic);
            bool collapseCols = (group.type != GroupType.Dynamic && group.type != GroupType.Environment);
            ...
            if (collapseMesh)
            {
                ...
                // 2 = Transparent = Fast track to avoid normal mapping when it isn't required.
                ...
                AnalyzeMeshes(viewMatrices, projectionMatrices, viewPositions, viewDirections, viewFrustums, instances, partMeshCount, partSurfaceMeshCount, canRemoveTubesAndPins, canRemoveKnobs, canRemoveCompletelyInvisible, group.lod);
                ...
                for (int i = 0; i < meshHelpers.Length; ++i)
                {
                    if (meshHelpers[i].vertices.Count > 0)
                    {
                        ...
                        Mesh m = new Mesh();
                        meshHelpers[i].ToMesh(m, group.lightmapped);
