...
namespace LEGOModelImporter
{
    ...
    public static class LDrawConverter
    {
        ...
        public static XmlDocument ConvertLDrawToLXFML(Stream ldrawInput, string name)
        {
            ...
            foreach (XmlNode transformationNode in transformationNodes)
            {
                ...
                else
                {
                    ...
                    {
                        Debug.LogWarning($"Duplicate transformation: {transformationNode.Attributes["ldraw"].Value}. Values differ! Ignoring new value");
                    }
        ...
        public static Dictionary<string, List<string>> GetErrors()
        {
            ...
            {
                errors.Add("The following assemblies could not be found. Replace with individual parts if possible in Studio, or add assemblies in ldrawassembly.xml translation file.", missingAssemblies);
            }
            if (missingDecorations.Count > 0)
            {
                errors.Add("The following bricks contained decorations, which have been removed. Add them manually after creation.", missingDecorations);
            }
            if (missingStickers.Count > 0)
            {
                errors.Add("The following bricks contained stickers, which have been removed. There is no support for stickers from Studio.", missingStickers);
            }
            if (missingMaterials.Count > 0)
            {
                errors.Add("The following materials could not be found. Use alternative materials if possible in Studio, or add mapping to known material in ldraw.xml translation file.", missingMaterials);
            }
            if (legacyMaterials.Count > 0)
            {
                errors.Add("The following materials were only found as legacy materials. Use alternative materials if possible in Studio.", legacyMaterials);
            }
            if (missingParts.Count > 0)
            {
                errors.Add("The following parts could not be found. Use alternative parts if possible in Studio, or add mapping to known part in ldraw.xml translation file.", missingParts);
            }
            if (changedParts.Count > 0)
            {
                errors.Add("The following parts were changed to an alternative part.", changedParts);
            }
            if (legacyParts.Count > 0)
            {
                errors.Add("The following parts were only found as legacy parts. Colliders and connectivity information will not be available for these parts. Use alternative parts if possible in Studio.", legacyParts);
            }
            if (missingTransformations.Count > 0)
            {
                errors.Add("The following parts have no transformation in ldraw.xml translation file. Use alternative parts if possible in Studio, or add them manually.", missingTransformations);
            }
        ...
        private static void AddPartToBrick(Brick brick, string ldrawID, Matrix4x4 transformation, string materialID, string fullName)
        {
            ...
            {
                missingMaterials.Add($"{ldrawID} => {materialID}");
            } else if (materialExistence == MaterialUtility.MaterialExistence.Legacy)
            {
                legacyMaterials.Add($"{ldrawID} => {materialID}");
            }
            ...
            if (patternSplit.Length > 1)
            {
                ...
                missingDecorations.Add(fullName);
            }
            ...
            if (partExistenceResult.existence == PartUtility.PartExistence.None)
            {
                ...
                if (versionSplit.Length > 1)
                {
                    ...
                    {
                        // Missing part.
                        missingParts.Add($"{fullName} => {designID}");
                    }
                    else
                    {
                        // Changed part.
                        changedParts.Add($"{fullName} => {designID}");
                        ...
                        if (partExistenceResult.existence == PartUtility.PartExistence.Legacy)
                        {
                            ...
                            legacyParts.Add($"{fullName} => {partExistenceResult.designID}");
                        }
                    }
                }
                else
            ...
            {
                missingTransformations.Add(designID);
            }
