...
namespace Unity.LEGO.Tutorials
{
    ...
    class BuildingCriteria : ScriptableObject
    {
        ...
        {
            if (!ExplodeInstance) //needed because IET doesn't properly clean up the current tutorial when interrupting a tutorial
            {
                return false;
            }
            System.Collections.Generic.HashSet<Action> actions = touchTriggerConnectedToAltar.GetTargetedActions();
            return actions.ElementAt(0) == ExplodeInstance.GetComponent<Action>();
        }
    }
}
