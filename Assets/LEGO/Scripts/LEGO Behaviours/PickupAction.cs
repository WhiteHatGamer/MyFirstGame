    ...
    public class PickupAction : Action
    {
        ...
        protected override void Start()
        {
            ...
            if (IsPlacedOnBrick())
            {
                ...
                foreach (var brick in m_ScopedBricks)
                {
                    foreach (var part in brick.parts)
                    {
                        if (!part.legacy && part.connectivity)
                        {
                            ...
                            {
                                field.DisconnectInverse(m_ScopedBricks);
                            }
                        }
                    }
                }
                ...
                foreach (var brick in m_ScopedBricks)
                {
                    brick.transform.RotateAround(transform.position + transform.TransformVector(m_BrickPivotOffset), Vector3.up, initialRotaion);
                }
            }
        }
        ...
        protected void Update()
        {
            if (m_Active)
            {
                ...
                foreach (var brick in m_ScopedBricks)
                {
                    ...
                    brick.transform.RotateAround(transform.position + transform.TransformVector(m_BrickPivotOffset), Vector3.up, k_AngularSpeed * Time.deltaTime);
                }
                m_Offset += delta;
